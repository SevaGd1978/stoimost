// ---------- State ----------
        ...
        function updateUI(options = {}) {
            ...
            perKm.innerHTML = `${formatPerKm(data.perKm)} <span class="rub">₽/км</span>`;
        ...
        function renderHistory() {
            ...
            reversed.forEach((item, idx) => {
                ...
                const dateStr = date.toLocaleDateString('ru-RU') + ' ' + date.toLocaleTimeString('ru-RU', { hour: '2-digit',
                    minute: '2-digit' });
                html += `
                        <div class="history-item" onclick="loadFromHistory(${realIdx})" title="Загрузить расчёт" style="cursor:pointer;">
                            <div class="left">
                            ...
                            <div style="display:flex;align-items:center;gap:12px;">
                                ...
                                <button class="delete-btn" onclick="event.stopPropagation(); deleteHistory(${realIdx})" title="Удалить">✕</button>
                            </div>
                        </div>
                    `;
            });
        ...
        function loadFromHistory(index) {
            ...
            const fuelPrice = item.fuelPrice !== undefined ? item.fuelPrice : 86.8;
            const fuelConsumption = item.fuelConsumption !== undefined ? item.fuelConsumption : 30;
            const platonRate = item.platonRate !== undefined ? item.platonRate : 6;

            kmInput.value = item.km;
            fuelPriceInput.value = fuelPrice;
            fuelConsumptionInput.value = fuelConsumption;
            platonRateInput.value = platonRate;
            fuelInput.value = item.fuel;
            salaryInput.value = item.salary;
            platonInput.value = item.platon;
            travelInput.value = item.travel;

            // Сохраняем ручные правки из истории, не перезаписывая авторасчётом
            const expectedFuel = computeFuel(item.km, fuelPrice, fuelConsumption);
            const expectedSalary = computeSalary(item.km);
            const expectedPlaton = computePlaton(item.km, platonRate);

            state.editedFuel = Math.abs(item.fuel - expectedFuel) > 1;
            state.editedSalary = Math.abs(item.salary - expectedSalary) > 1;
            state.editedPlaton = Math.abs(item.platon - expectedPlaton) > 1;

            autoFuelCheck.checked = !state.editedFuel;
            autoSalaryCheck.checked = !state.editedSalary;
            autoPlatonCheck.checked = !state.editedPlaton;

            fuelInput.classList.toggle('auto-calc', !state.editedFuel);
            fuelInput.classList.toggle('is-edited', state.editedFuel);
            salaryInput.classList.toggle('auto-calc', !state.editedSalary);
            salaryInput.classList.toggle('is-edited', state.editedSalary);
            platonInput.classList.toggle('auto-calc', !state.editedPlaton);
            platonInput.classList.toggle('is-edited', state.editedPlaton);

            updateUI({ skipHistory: true });
        ...
        function syncManualFlag(input, checkbox, editedKey, expected) {
            const current = parseFloat(input.value) || 0;
            if (Math.abs(current - expected) > 1) {
                state[editedKey] = true;
                input.classList.add('is-edited');
                input.classList.remove('auto-calc');
                checkbox.checked = false;
            } else {
                state[editedKey] = false;
                input.classList.remove('is-edited');
                input.classList.add('auto-calc');
        ...
        function bindInputs() {
            ...
            [kmInput, fuelPriceInput, fuelConsumptionInput, platonRateInput].forEach(inp => {
                inp.addEventListener('input', onKmOrRateChange);
            });
            ...
            fuelInput.addEventListener('input', () => {
                if (autoFuelCheck.checked) {
                    ...
                    const price = parseFloat(fuelPriceInput.value) || 0;
                    const cons = parseFloat(fuelConsumptionInput.value) || 0;
                    syncManualFlag(fuelInput, autoFuelCheck, 'editedFuel', computeFuel(km, price, cons));
                }
                updateUI();
            });
            ...
            salaryInput.addEventListener('input', () => {
                if (autoSalaryCheck.checked) {
                    const km = parseFloat(kmInput.value) || 0;
                    syncManualFlag(salaryInput, autoSalaryCheck, 'editedSalary', computeSalary(km));
                }
                updateUI();
            });
            ...
            platonInput.addEventListener('input', () => {
                if (autoPlatonCheck.checked) {
                    const km = parseFloat(kmInput.value) || 0;
                    const rate = parseFloat(platonRateInput.value) || 0;
                    syncManualFlag(platonInput, autoPlatonCheck, 'editedPlaton', computePlaton(km, rate));
                }
                updateUI();
            });
